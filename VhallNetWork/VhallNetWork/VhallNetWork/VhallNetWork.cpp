...
HANDLE HttpManager::gDownLoadFile = nullptr;
std::atomic_bool HttpManager::gbIsDownLoadThreadRuning = false;
...
bool HttpGetRunTask::IsDownloadFile() {
   return  mTaskParam.mbIsDownLoadFile;
}
...
void HttpGetRunTask::RunTask() {
   if (mTaskParam.mbIsDownLoadFile && mTaskParam.mbIsNeedSyncWork) {
      HttpDownLoadFile();
   } else {
      HttpRequest();
   }
    ...
    if (!mbIsInit) {
        ...
        */
        ...
        if (mDownLoadThread == nullptr) {
           gbIsDownLoadThreadRuning = true;
           gDownLoadFile = ::CreateEvent(NULL, FALSE, FALSE, NULL);
           mDownLoadThread = new std::thread(HttpManager::DownLoadThreadProcess, this);
        }
    }
...
void HttpManager::DownLoadProcessTask() {
   std::shared_ptr<HttpGetRunTask> event = nullptr;
   if (bManagerThreadRun) {
      std::unique_lock<std::mutex> lock(mDownLoadTaskListMutex);
      if (mDownLoadTaskList.size() > 0) {
         event = mDownLoadTaskList.front();
      }
      else {
         return;
      }
   ...
   if (event) {
      event->RunTask();
      std::unique_lock<std::mutex> lock(mDownLoadTaskListMutex);
      mDownLoadTaskList.pop_front();
   }
}

DWORD WINAPI HttpManager::DownLoadThreadProcess(LPVOID p){
   LOGD("ThreadProcess");
   while (gbIsDownLoadThreadRuning) {
      DWORD ret = WaitForSingleObject(gDownLoadFile, MAX_THREAD_WAIT_TIME);
      if (p) {
         HttpManager* sdk = (HttpManager*)(p);
         if (sdk) {
            sdk->DownLoadProcessTask();
         }
      }
   }
...
void HttpManager::HttpGetRequest(HTTP_GET_REQUEST& httpGetParam, EventCallback fun) {
    ...
    if (bManagerThreadRun) {
        ...
        if (task->IsDownloadFile()) {
           if (gbIsDownLoadThreadRuning) {
              std::unique_lock<std::mutex> lock(mDownLoadTaskListMutex);
              mDownLoadTaskList.push_back(task);
              if (gDownLoadFile) {
                 ::SetEvent(gDownLoadFile);
              }
           }
        }
        else {
