...
public:
    ...
    void DownLoadProcessTask();
    static DWORD WINAPI  DownLoadThreadProcess(LPVOID p);
...
private:
   ...
   // std::list<HttpGetRunTask*> mTaskList;
    ...
    static HANDLE gDownLoadFile;
    static std::atomic_bool gbIsDownLoadThreadRuning;
    std::thread* mDownLoadThread = nullptr;
    std::mutex mDownLoadTaskListMutex;
    std::list<std::shared_ptr<HttpGetRunTask>> mDownLoadTaskList;
